    ...
    public static void configureLogging() throws ExceptionInInitializerError {
        ...
        if (System.getProperty("java.util.logging.config.file") == null) {
            ...
            if (!TextUtil.isNullOrEmpty(System.getenv(OR_LOGGING_CONFIG_FILE))) {
                ...
                if (Files.isReadable(Paths.get("/deployment/monitoring/logging.properties"))) {
                    try (InputStream is = Files.newInputStream(Paths.get("/deployment/monitoring/logging.properties"))) {
                        System.out.println("Using logging configuration: /deployment/monitoring/logging.properties");
                        LogManager.getLogManager().readConfiguration(is);
                    } catch (Exception ex) {
                        throw new ExceptionInInitializerError(ex);
                    }
